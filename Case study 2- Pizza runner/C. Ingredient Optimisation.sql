...
join json_table(trim(replace(json_array(extras), ',', '","')), '$[*]' columns (extras varchar(50) path '$')) as jt
join pizza_toppings
...
join json_table(trim(replace(json_array(exclusions), ',', '","')),'$[*]' columns (exclusions varchar(50) path '$')) as jt
join pizza_toppings 
